  ...
  # Ronald Zom 3, M.K. van Ittersum 2, P.J. Gerber 1,4, and I.J.M. de Boer 1.                         #
  #                                                                                     #
  #                                                                                     #
  # 1                                                                                   #
  ...
  #                                                                                     #
  # 3                                                                                   #
  # Animal Nutrition Group, Wageningen Livestock Research                               #
  # Wageningen University & Research                                                    #
  # P.O. Box 338                                                                        #
  # 6700 AH  Wageningen                                                                 #
  # The Netherlands                                                                     #
  ...
  #                                                                                     #
  # This specific version of LiGAPS-Dairy is a basic version for those interested in    #
  # the charactistics of the model.                                                     #
  #                                                                                     #
  ...
  ## Climate (used to read weather data file, defining factor)
  ...
  POTENTIAL <- 1   # Potential diet 1 = yes, 0 = no
                   # The potential diet consists of 19.5% soybean meal, 31.8% wheat,
                   # and 48.6% good quality hay, and it is fed ad libitum.
  FEEDQUALITY <- 0 # Ad libitum diet, i.e. without feed quantity limitation 1 = yes, 
  ...
  # of the year. Stable (STABLE) is specified l.97
  ...
  # Diet numbers. This option can be used to adjust proportions of feeds in the diet,
  # although these proportions can be included in feed input files (.csv) as well.
  FEEDNR <- rep(1,1000) 
  
  COUNT = 0           # Initial number for counting the cows 
  ...
  # Each repeat of this loop is the simulation of one cow.
  
  zseries <- c(1:1) 
  OUTPUTHERDS1    = matrix(nrow=max(zseries), ncol=2)
  #######################################################################################
  ...
  # Cattle management
      ...
      WEANINGTIME = 305         # Length of the lactation period (days)
      GESTINTERVAL = 365        # Minimum calving interval (days)
      STDOYBASE = 50            # Calving day (Julian day) for reproductive animals
      STDOYBASE1 = 1            # Day of the year (Julian day) in which the first animal 
                                # is born
      STDOY = 1                 # Day at which the HOUSING and WEATHER matrices are used.
                                # Most weather input files start at the 1st of January, 
                                # which means that STDOY = 1 corresponds to this date. 
  ...
  if(LOCATION == 'NETHERLANDS') 
    WEATHER <-read.csv("C:/R/NLDLEL19912018_ext.csv", head=TRUE,sep=",") 
  ...
  # weight, milk prod., 2x start milk param. (milk from mother)
                     30.85, 0.08635, 700, 1, 30.85, 0.08635,
  # 17-19   fraction TBW fertility, maintenance factor, min perc. ((subc. + interm. fat)/TBW) 
  # for gestation
                     0.49, 1.09, 0.10,                                         
  # 20-23   lipid fraction bone parameter, carcass fraction, muscle:bone ratio, min. cond. 
  ...
  # Input data on feed quality and feed quantity have to be saved in a directory. Each file 
  # contains feed data for one cow. The directories must correspond to the correct 
  # directories on the computer drive. The diets corresponds to the measured feed quality and 
  # feed intake.
  
  LISTFEEDFILES <- list.files("C:/R/Feed")
  
  # Selects the name of a feed input file in the z-loop    
  FILENAME <- paste("C:/R/Feed/", LISTFEEDFILES[z], sep = "")
  ...
  if(POTENTIAL == 1) POTENTIALFEED <- read.csv(file="C:/R/Feedinput.csv", header=T) 
  if(POTENTIAL == 1) FEEDINPUT <- POTENTIALFEED
  
  # The feed intake of feed 1, 2 and 3 are both set at 50 kg DM per animal per day under feed 
  ...
  if(FEEDQUALITY == 1) FEEDINPUT$Feed3QNTY <- rep(50,length(FEEDINPUT$Feed3QNTY))
  ...
  F1i <- cbind(c(0.0001044*FEEDINPUT$Feed1Dig^2 - 0.0195376*FEEDINPUT$Feed1Dig + 1.1501084),
               ...
               60, 376.00,  FEEDINPUT$Feed1CP*0.20, FEEDINPUT$Feed1CP*0.60, 385.00, 0.300, 0.040, 0.085, 0.035, 141.00, 0.470, 0.50,
  ...
  F2i <- cbind(c(0.0001044*FEEDINPUT$Feed2Dig^2 - 0.0195376*FEEDINPUT$Feed2Dig + 1.1501084),
               ...
               60, 376.00,  FEEDINPUT$Feed1CP*0.18, FEEDINPUT$Feed1CP*0.55, 385.00, 0.300, 0.040, 0.085, 0.035, 141.00, 0.470, 0.50,
  ...
  F3i <- cbind(c(0.0001044*FEEDINPUT$Feed3Dig^2 - 0.0195376*FEEDINPUT$Feed3Dig + 1.1501084),
               ...
               60, 376.00,  FEEDINPUT$Feed1CP*0.15, FEEDINPUT$Feed1CP*0.45, 385.00, 0.300, 0.040, 0.085, 0.035, 141.00, 0.470, 0.50,
  ...
  if(FEEDNR[z] == 1) FEED1fr <- 0.20 else if(FEEDNR[z] == 2) FEED1fr <- 0.50 else 
    if(FEEDNR[z] == 3) FEED1fr <- 0.50 else if(FEEDNR[z] == 4) FEED1fr <- 0.50 else 
      if(FEEDNR[z] == 5) FEED1fr <- 0.50    
  if(FEEDNR[z] == 1) FEED2fr <- 0.80 else if(FEEDNR[z] == 2) FEED2fr <- 0.50 else 
  ...
  FEED4fr <- 1.00 # The diet can consist of 100% FEED4 (FEED 4 is not used).
  
  # Feed quantity available (kg DM per animal per day) for feed type 1-3.    
  FEEDQNTYTOT <- FEED1QNTY + FEED2QNTY + FEED3QNTY 
  ...
  NEm             = 365                 # [78] NE for maintenance (kJ NE kg EBW-0.75, for B. 
  ...
  MILKDIG         = 0.95            # [40] digestible fraction of milk, based on energy 
                                    # content of milk
  NEEFFMILK       = 0.890933        # [74] efficiency of conversion of NE to milk (on an 
                                    # energy basis)
  PROTEFFMILK     = 0.71            # [82] protein efficiency for milk production (CSIRO, 
                                    # 2007)
  ...
  TTDIGINSC       = 0.97            # [31] fraction total tract digestibility of insoluble, 
                                    ...
                                    #      2014)
  DETOME          = 0.82            # [26] conversion from digestible energy (DE) to 
  ...
  # with dairy cattle (up to the farm gate). Outcomes of GHG emissions are not presented 
  # in the paper describing LiGAPS-Dairy.
  ...
  # Note that some of the input parameters depend on the location, climate conditions and
  # stable design and manure management.
  INPUTDATA <- read.csv("C:/R/Input GHG module.csv", head=T)
  ...
  # Gestation (female calves can conceive if six requirements are met)
  ...
  if(MILKDAY[i,j] >0 && RatioLG[i,j] <0)  ENFEEDLACT[i,j] <- min(NEMILKCOW[i,j] * 
     min(1,sqrt(MISCFATTISACT[i,j]/MISCFATTIS[i,j])+0.25), 
     ENFEEDVAR[i+1,j]*(1+(2/3)*abs(RatioLG[i,j])))  
  ...
  # End of the module on GHG emissions        
  ###########################################################################################
  ...
  # Creates a vector with days of birth of the calves      
  ...
  OUTPUTHERDS1[z,] <- c(OUTPUTHERD[3,27]/OUTPUTHERD[3,6],OUTPUTHERD[3,27]/(OUTPUTHERD[3,8]-2))
  ...
  # Average simulated weight (kg total body weight)
  SIMULATEDWEIGHTlact <- mean(TBWACT[1:ENDDAY[1],1])

  # Daily fat- and protein-corrected milk production (kg FPCM per day) 

  # Simulated FPCM production (kg FPCM)
  SIMULATEDFPCM <- c(MILKPRODACTFPCM[1:ENDDAY[1]],1)
  
  # Simulated feed intake (kg DM)
  SIMULATEDFIlacttotal <- sum(na.omit(c(FEEDQNTY[min(FEEDINPUT$Time):max(FEEDINPUT$Time),1]))) 
  
  # Simulated feed intake (kg DM per day)
  SIMULATEDFI <- c(FEEDQNTY[min(FEEDINPUT$Time):max(FEEDINPUT$Time),1]) 
  
  # Simulated genetic potential milk production per lactation (kg FPCM per lactation)
  SIMULATEDFPCMPOTlact <- sum(c(MILKPRODPOTFPCM[1:ENDDAY[1],1])) 

  # Collection of performance parameters for individual cows. 
  
  EVALUATIONCOW <- c(NA,SIMULATEDFPCMPOTlact,NA,
  ...
  NRHEATSTRESS <- length(which(HEATSTRESS[1:ENDDAY[1],1] < 0)) 
  # Days with heat stress (0 = no heat stress; 1 = heat stress)
  HEATSTRESSfact <- HEATSTRESS[1:ENDDAY[1],1]
  HEATSTRESSfact[HEATSTRESSfact<0] <-1 
    
  # Average daily maximum temperature (degrees Celsius)
  TAVGCexpmax <- WEATHER$MAXT[1:ENDDAY[1]]
  # Average daily minimum temperature (degrees Celsius)
  TAVGCexpmin <- WEATHER$MINT[1:ENDDAY[1]]
  # Average daily maximum temperature during days at which heat stress occurs (degrees 
  # Celsius)
  TempHEATSTRESS <- TAVGCexpmax[REDHP[1:ENDDAY[1],1]<0]
  ...
  # Number of days with heat stress (#)
  NRCOLDSTRESS <- length(which(COLDSTRESS[1:ENDDAY[1],1] > 0))
  # Average daily minimum temperature during days at which cold stress occurs (degrees 
  # Celsius)
  TempCOLDSTRESS <- TAVGCexpmin[COLDSTRESS[1:ENDDAY[1],1]>0]
  # Days with cold stress (0 = no cold stress; 1 = cold stress)
  COLDSTRESSfact <- COLDSTRESS[1:ENDDAY[1],1]
  COLDSTRESSfact[COLDSTRESSfact>0] <-1
  COLDSTRESSfact[COLDSTRESSfact<0] <-0
  
  # Rumen fill (fraction maximum digestion capacity)
  FILLGIT1 <- FILLGIT[1:ENDDAY[1],1]
  ...
  # Number of days where digestion capacity is limiting (#)
  ...
  NELIM[NELIM<-0.00001] <- NA
  # Number of days with energy deficiency (#)
  NRNELIM <- length(which(NELIM[1:ENDDAY[1]] >-0.00001))
  # Days with energy deficiency (0 = no limitation; 1 = limitation)
  NEDEFfact <- NELIM[1:ENDDAY[1]]
  NEDEFfact[is.na(NEDEFfact)] <-0
  
  # Number of days where the genotype defines production (#)
  NRGENDEF <- length(c(1:ENDDAY[1]))-NRHEATSTRESS-
    NRCOLDSTRESS-NRFILLGIT-NRNELIM-NRPROTGRAPH
  # Days where the genotype defines milk production (0 = not defining; 1 = defining)
  GENfact <- rep(1,length(1:ENDDAY[1]))-
    HEATSTRESSfact-COLDSTRESSfact-FILLGITfact-PROTDEFfact-NEDEFfact
  
  # Vectors are extended to 4000 days with zeros, to make the length of the vector
  # the same for all cows.
  GENfact <- c(c(GENfact), c(rep(0,4000-length(GENfact))))
  HEATSTRESSfact <- c(c(HEATSTRESSfact), c(rep(0,4000-length(HEATSTRESSfact))))
  COLDSTRESSfact <- c(c(COLDSTRESSfact), c(rep(0,4000-length(COLDSTRESSfact))))
  FILLGITfact <- c(c(FILLGITfact), c(rep(0,4000-length(FILLGITfact))))
  NEDEFfact <- c(c(NEDEFfact), c(rep(0,4000-length(NEDEFfact))))
  PROTDEFfact <- c(c(PROTDEFfact), c(rep(0,4000-length(PROTDEFfact))))
  ...
  # production (%)
  relDEFLIMFACTORS <- DEFLIMFACTORS/(length(c(1:ENDDAY[1])))*100
  
  # The code below collect information for multiple cows 
  
  # Defining and limiting factors for all cows 
  ...
  # Number of cows simulated (#)
  # The total number of cows to be simulated can be found in zseries.
  ...
  legend("topleft", c("Genetic potential TBW", "Simulated TBW"),
         ...
         bty = "n")
  par(new=T)
  plot(MILKPRODACTFPCM[1:maxgr,1]~c(1:maxgr), type = "l", axes = FALSE, bty = "n", 
       xlab = "", ylab = "", ylim=c(0,100), las = 1, yaxs="i", xaxs="i")
  axis(side=4, at = seq(0,100, by= 10), las=1)
  mtext("Milk production (kg FPCM per day)", side=4, line=2.5, cex = 0.7)
  text(x=3500,y=90, paste("Cow ",z))
  ...
  par(new=T)
  barplot(as.matrix(bars), space = 0, border = NA, xaxt = "n", ylim=c(0,40), xaxs= "i",
          col= c("gold","chartreuse3","darkolivegreen2"), las=1,
          ylab = expression(paste("Feed intake (kg day"^"-1"*")")))
  
  legend("topleft",
         legend=c("Feed 1","Feed 2","Feed 3"), 
         fill = c("gold","chartreuse3","darkolivegreen2"), 
         cex=1, bty = "n")
  ...
  axis(2,at = c(0.5:5.5), labels = c("protein", "energy","feed int. cap.", "cold stress", 
